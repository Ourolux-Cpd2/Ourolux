...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±º Programa ³ FA070CA4() ³ Autor ³ Claudino P Domingues ³ Data ³ 02/10/13 º±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±º Funcao Padrao ³ FINA070                                                º±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±º Desc.    ³ Sera executado apos confirmacao do cancelamento/exclusao da º±±
±±º          ³ baixa do contas a receber. Envia Workflow. 				   º±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	...
	If Upper(Alltrim(cUserName)) $ GetMv("FS_EXCFIN1") .OR. Upper(Alltrim(cUserName)) $ GetMv("FS_EXCFIN2")
		...
		If _nDayDif > 0
			...
			If (_nDayDif - _nDayNot) > 2
				ApMsgStop("Usuário sem acesso, só pode ser cancelado dentro de dois dias uteis. Favor verificar com o seu gestor.","FA070CA4")
				_lRet := .F.
			Else
				...
				If (_nDayDif - _nDayNot) == 2
					...
					If !(Upper(Alltrim(cUserName)) $ GetMv("FS_EXCFIN2"))
						ApMsgStop("Usuário sem acesso, só pode ser cancelado dentro de um dia útil. Favor verificar com o seu gestor","FA070CA4")
						_lRet := .F.
					EndIf
				EndIf
			EndIf
		EndIf	
	Else
		ApMsgStop("Usuario sem permissão para executar essa operação! Favor verificar com o seu gestor.","FA070CA4")
		_lRet := .F.
	EndIf
EndIf	
...
If _lRet
	...
	If Empty(_cMotCanc)
		ApMsgStop("Por favor informar o motivo do cancelamento, sem o motivo não é possivel cancelar a baixa.","FA070CA4")
		_lRet := .F.
	EndIf
