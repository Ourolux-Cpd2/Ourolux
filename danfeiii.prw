...
#DEFINE MAXITEMC  040                                                // Máxima de caracteres por linha de produtos/serviços // CUSTOMIZACAO OUROLUX
#DEFINE MAXMENLIN 140                                                // Máximo de caracteres por linha de dados adicionais
...
#DEFINE MAXVALORC 012                                                // Máximo de caracteres por linha de valores numéricos // CUSTOMIZACAO OUROLUX

// CUSTOMIZAÇÂO OUROLUX - Roberto Souza
// Tamanho das colunas dos itens       
...
private oRetNF
Aviso("DANFE III","Compatibilização Protheus 12 - 31/07/2018",{"Ok"})
...
Local cEstoque  := 'S' // rogerio para consistencia de movimentacao da TES/estoque a ser feito por item das notas fiscais de saida Wadih em 03/02/2021
Private cUsrAutE:= ALLTRIM(GetMv("ES_USRNFES")) // Rogerio, parametro para informar o Usuario autorizado a imprimir DANFE SEM MOVIMENTAR ESTOQUE 08/02/21
...
If lIsLoja
	...
	MV_PAR05 := 2	//Frente e Verso - 2:Nao
			...
			If  Empty((cAliasSF3)->F3_DTCANC) .And. &cWhere //.And. AModNot((cAliasSF3)->F3_ESPECIE)=="55"
				...
				EndIf
					...
					if cEstoque = 'S' .OR. (cEstoque = 'N' .and. (Upper(Alltrim(cUserName)) $ cUsrAutE ))
						...
						Set Device To Screen 
						ApMsgStop("Nota fiscal: "+(cAliasSF3)->F3_NFISCAL+" série "+(cAliasSF3)->F3_SERIE+ ' tem produtos que nao movimenta estoque, Impressao nao permitida!', 'ATENÇÃO' )
					EndIf
				Endif
			EndIf
...
Local __cProd		:= "" // CUSTOMIZACAO OUROLUX CLAUDINO
Local __cItem		:= "" // CUSTOMIZACAO OUROLUX CLAUDINO
Local __cLote		:= "" // CUSTOMIZACAO OUROLUX CLAUDINO
Local _nMaxLot      := 10 // CUSTOMIZACAO OUROLUX CLAUDINO
Local _cAuxLot      := "" // CUSTOMIZACAO OUROLUX CLAUDINO
...
Private nDifC 		:= 5 //Customizacao Ourolux - Diferença na impressão/coluna itens 
...
If nFaturas > 0
	For nX := 1 To 3
		aAux := {}
		For nY := 1 To Min(12, nFaturas) //	For nY := 1 To Min(9, nFaturas)  - Original - Customizacao Isaias
			Do Case
				Case nX == 1
					If nFaturas > 1
...
For nZ := 1 To nLenDet
	...
	*/
    ...
    If MV_PAR04==1 .And. (oNfe:_NFE:_INFNFE:_IDE:_TPNF:TEXT=="0")
    	
    	__cProd := PADR(oDet[nX]:_Prod:_cProd:TEXT,15)
    	__cItem := StrZero(Val(oDet[nX]:_nItem:TEXT),4)
        
 		dbSelectArea("SD1")
    	SD1->(dbSetOrder(1)) // D1_FILIAL+D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+D1_COD+D1_ITEM
		If SD1->(dbSeek(xFilial("SD1")+aNota[5]+aNota[4]+aNota[6]+aNota[7]+__cProd+__cItem))
			__cLote := SD1->D1_LOTECTL
    	EndIf
    
    ElseIf MV_PAR04==2 .And. (oNfe:_NFE:_INFNFE:_IDE:_TPNF:TEXT=="1")
    	
    	__cProd := PADR(oDet[nX]:_Prod:_cProd:TEXT,15)
    	__cItem := StrZero(Val(oDet[nX]:_nItem:TEXT),2)
 
    	SD2->(dbSetOrder(3))
		If SD2->(dbSeek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA+__cProd+__cItem))
			__cLote := SD2->D2_LOTECTL
    	EndIf
...
//	nPerDesc	:= ((nDesc*100)/nVTotal)
	...
	aadd(aItens,{;
		...
		__cLote,; //SubStr(Alltrim(__cLote),1,_nMaxLot),; // CUSTOMIZACAO OUROLUX CLAUDINO
...
nPos2Col := 185
For Nx := 1 to 12
	oDanfe:Box(nLine+275,nPos1Col,nLine+310,nPos2Col) 
	if Nx == 12
		exit
	elseif Nx == 11
		nPos1Col += 59
...
If Len(aFaturas) >0
	...
	For nY := 1 To Min(nFaturas,nMaxFat)
		nX++
		oDanfe:Say(nLine+287,nColuna,aFaturas[nX][nY],oFont08:oFont)
		nX++
		oDanfe:Say(nLine+296,nColuna,aFaturas[nX][nY],oFont08:oFont)
		nX++
		oDanfe:Say(nLine+305,nColuna,aFaturas[nX][nY],oFont08:oFont) 
...
aAuxCabec := {;
	"COD. PROD",;
	"DESCR PROD",;
	"LOTE",; // CUSTOMIZACAO OUROLUX CLAUDINO
	"NCM/SH",;
...
For nY := 1 To nLenItens
	...
	Else
		...
		nAuxH2 := len(aAux[1][20][nY]) + (aColProd[20][1] + ((aColProd[20][2] - aColProd[20][1]) - RetTamTex(aAux[1][20][nY], oFont07:oFont, oDanfe)))
		oDanfe:Say(nLinha, nAuxH2, aAux[1][20][nY], oFont07:oFont)
	EndIf
...
Local _cObs1     := "" // CUSTOMIZACAO OUROLUX CLAUDINO
...
// INICIO CUSTOMIZACAO OUROLUX CLAUDINO
// 1º CANHOTO
oDanfe:Box( 000 , 000 , 030 , 060 ) // BOX 1 NUMERO NFE
oDanfe:Box( 030 , 000 , 285 , 025 ) // BOX 1 RECEBEMOS DE ELETROMEGA
oDanfe:Box( 030 , 025 , 225 , 060 ) // BOX 1 ASSINATURA
oDanfe:Box( 225 , 025 , 285 , 060 ) // BOX 1 DATA REC NFE

oDanfe:Say( 007 , 002 , "NF-e", oFont08N:oFont , , , 000 )
oDanfe:Say( 016 , 002 , "Nº "+StrZero(Val(oIdent:_NNf:Text),9), oFont08:oFont, , , 000 )
oDanfe:Say( 025 , 002 , "SÉRIE "+SubStr(oIdent:_Serie:Text,1,3), oFont08:oFont, , , 000 )

_cObs1 := "RECEBEMOS DE "+NoChar(oEmitente:_xNome:Text,lConverte)+" OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA AO LADO"
oDanfe:Say( 280 , 010 , SubString(_cObs1,1,13+Len(oEmitente:_xNome:Text)), oFont08N:oFont, , , 270 )
oDanfe:Say( 280 , 020 , Substring(_cObs1,15+Len(oEmitente:_xNome:Text),Len(_cObs1)), oFont08N:oFont, , , 270 )

oDanfe:Say( 220 , 030 , "IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR", oFont08N:oFont, , , 270 )
oDanfe:Say( 280 , 030 , "DATA RECEB", oFont08N:oFont, , , 270)

oDanfe:Say( 301 , 000, Replicate("- ",16), oFont08N:oFont, , , 000 )

// 2º CANHOTO
oDanfe:Box( 315 , 000 , 345 , 060 ) // BOX 2 NUMERO NFE
oDanfe:Box( 345 , 000 , 600 , 025 ) // BOX 2 RECEBEMOS DE ELETROMEGA
oDanfe:Box( 345 , 025 , 540 , 060 ) // BOX 2 ASSINATURA
oDanfe:Box( 540 , 025 , 600 , 060 ) // BOX 2 DATA REC NFE

oDanfe:Say( 322 , 002 , "NF-e", oFont08N:oFont , , , 000 )
oDanfe:Say( 331 , 002 , "Nº "+StrZero(Val(oIdent:_NNf:Text),9), oFont08:oFont, , , 000 )
oDanfe:Say( 339 , 002 , "SÉRIE "+SubStr(oIdent:_Serie:Text,1,3), oFont08:oFont, , , 000 )

oDanfe:Say( 595 , 010 , SubString(_cObs1,1,13+Len(oEmitente:_xNome:Text)), oFont08N:oFont, , , 270 )
oDanfe:Say( 595 , 020 , Substring(_cObs1,15+Len(oEmitente:_xNome:Text),Len(_cObs1)), oFont08N:oFont, , , 270 )

oDanfe:Say( 535 , 030 , "IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR", oFont08N:oFont, , , 270 )
oDanfe:Say( 595 , 030 , "DATA RECEB", oFont08N:oFont, , , 270)
// FIM CUSTOMIZACAO OUROLUX CLAUDINO
...
If nfolha>=1
	If lMv_Logod
		...
		oDanfe:SayBitmap(001,075,cLogoD,095,090)
	Else
		//oDanfe:SayBitmap(005,075,cLogo,095,096)
		oDanfe:SayBitmap(001,075,cLogo,095,090)
	EndIf
Endif
...
If nFolha == 1
	...
	nAux += aTamCol[2]
	AADD(aColProd, {nAux, nAux + aTamCol[3]}) // "LOTE"	
	nAux += aTamCol[3]
	...
	nAux += aTamCol[9]
	AADD(aColProd, {nAux, nAux + aTamCol[10]}) // "VLR TOTAL"
	nAux += aTamCol[10]
//	AADD(aColProd, {nAux, nAux + aTamCol[10]}) // "PER DESC"
 //	nAux += aTamCol[10]
	AADD(aColProd, {nAux, nAux + aTamCol[11]}) // "VLR DESC"
	nAux += aTamCol[11]
	AADD(aColProd, {nAux, nAux + aTamCol[12]}) // "VLR LIQ"
	nAux += aTamCol[12]
	AADD(aColProd, {nAux, nAux + aTamCol[13]}) // "TOT LIQ"
	nAux += aTamCol[13]
	AADD(aColProd, {nAux, nAux + aTamCol[14]}) // "BC.ICMS"
	nAux += aTamCol[14]
	AADD(aColProd, {nAux, nAux + aTamCol[15]}) // "BC.ICMS ST"
	nAux += aTamCol[15]
	AADD(aColProd, {nAux, nAux + aTamCol[16]}) // "VLR ICMS"
	nAux += aTamCol[16]
	AADD(aColProd, {nAux, nAux + aTamCol[17]}) // "VLR ICMS ST"
	nAux += aTamCol[17]
	AADD(aColProd, {nAux, nAux + aTamCol[18]}) // "VALOR IPI"
	nAux += aTamCol[18]
	AADD(aColProd, {nAux, nAux + aTamCol[19]}) // "ICMS"
	nAux += aTamCol[19]
	AADD(aColProd, {nAux, nAux + aTamCol[20]}) // "IPI"
	...
	oDanfe:Say(nLine+462, nAux + 2,"LOTE", oFont08N:oFont)
	nAux += aTamCol[3]
...
//	nAux += aTamCol[10]
	...
	nAux += aTamCol[19]
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[20])
	oDanfe:Say(nLine+468, nAux + 2,"IPI", oFont08N:oFont)
	oDanfe:Box(nLine+454, nAux2, nLine+461, nAux2 + aTamCol[19] + aTamCol[20])
...
//	CUSTOMIZACAO OUROLUX PARA FIXAR TAMANHO DAS COLUNAS - INICIO
	aTamCol[01] := IT_COL01
	aTamCol[02] := IT_COL02
	aTamCol[03] := IT_COL03
	aTamCol[04] := IT_COL04
...
Static Function RetTamTex(cTexto, oFont, oPrinter)
	...
	// INICIO CUSTOMIZACAO OUROLUX CLAUDINO
	If nTamanho == 44 // 100.00,00
		nTamanho := 37
	ElseIf nTamanho == 39 // 10.000,00
	    nTamanho := 33
	ElseIf nTamanho == 34 // 1.000,00
		nTamanho := 29
	ElseIf nTamanho == 27 // 100,00 // 1,0000
		nTamanho := 23
	ElseIf nTamanho == 22 // 10,00
		nTamanho := 19  
	ElseIf nTamanho == 17 // 1,00
		nTamanho := 15 
	ElseIf nTamanho == 32 // 10,0000
		nTamanho := 27
	ElseIf nTamanho == 37 // 100,0000
		nTamanho := 31
	EndIf
	// FIM CUSTOMIZACAO OUROLUX CLAUDINO
