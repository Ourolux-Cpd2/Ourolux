...
Local nReg   	:= 0
Local nomeprog	:= "TMKR03.PRW"			// nome do programa
...
If cNumAte <> Nil
	...
	Mv_Par04 := Ctod("31/12/49") //08/02/2021
	Mv_Par05 := cNumAte
	Mv_Par06 := cNumAte
Endif
...
While	(!Eof())							.AND.;
	...
	If dbSeek(xFilial("SUB")+SUA->UA_NUM)
		...
		//EndIf			
			...
			nTotGeral+= SUB->UB_VLRITEM   //Valor dos produtos + IPI
