...
±±³Programa  ³ SYMREL01 ³ Autor ³ Desenvolvimento B4B   ³ Data ³ 28/01/12 ³±±
...
oReport := TReport():New(cPerg,"RELATÓRIO DE FATURAMENTO X VENDEDOR X CLIENTE",cPerg, {|oReport| ReportPrint(oReport)},"RELATÓRIO DE FATURAMENTO X VENDEDOR X CLIENTE")
oReport:SetLandscape(.T.)
oSection1 := TRSection():New(oReport,OemToAnsi("FATURAMENTO X VENDEDOR X CLIENTE"),)
...
TRCell():New(oSection1,"Gerente" 		,/*Tabela*/,"Gerente"					,"@!"								,20	,/*lPixel*/,{|| cGerente })
TRCell():New(oSection1,"Supervisor" 	,/*Tabela*/,"Supervisor"  				,"@!"								,20	,/*lPixel*/,{|| cSupervi })
//TRCell():New(oSection1,"FilVend"    	,/*Tabela*/,"Filial de Vendas"			,"@!"								,TamSX3("A3_COD")[1]	,/*lPixel*/,{|| cFilVen })
TRCell():New(oSection1,"Vendedor" 		,/*Tabela*/,"Vendedor"					,"@!"								,06	,/*lPixel*/,{|| cVend })
TRCell():New(oSection1,"NomVend" 		,/*Tabela*/,"Nome Vend"		 			,"@!"								,15	,/*lPixel*/,{|| cNoVend })
TRCell():New(oSection1,"Cliente" 		,/*Tabela*/,RetTitle("D2_CLIENTE")		,PesqPict("SD2","D2_CLIENTE")		,09	,/*lPixel*/,{|| cCliente })
TRCell():New(oSection1,"cNomCli"	    ,/*Tabela*/,RetTitle("A1_NOME")			,PesqPict("SA1","A1_NOME")			,25						,/*lPixel*/,{|| cNomCli })
...
/*oBreak1 := TRBreak():New( oSection1,oSection1:Cell("Cliente"),"TOTAL CLIENTE")
TRFunction():New(oSection1:Cell(aFiltro[6][3]), ,"SUM", oBreak1,,,,.F.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[5][3]), ,"SUM", oBreak1,,,,.F.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[4][3]), ,"SUM", oBreak1,,,,.F.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[3][3]), ,"SUM", oBreak1,,,,.F.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[2][3]), ,"SUM", oBreak1,,,,.F.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[1][3]), ,"SUM", oBreak1,,,,.F.,.F.)     */

oBreak2 := TRBreak():New( oSection1,oSection1:Cell("Vendedor"),"TOTAL VENDEDOR")
TRFunction():New(oSection1:Cell(aFiltro[6][3]), ,"SUM", oBreak2,,,,.T.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[5][3]), ,"SUM", oBreak2,,,,.T.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[4][3]), ,"SUM", oBreak2,,,,.T.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[3][3]), ,"SUM", oBreak2,,,,.T.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[2][3]), ,"SUM", oBreak2,,,,.T.,.F.)               
TRFunction():New(oSection1:Cell(aFiltro[1][3]), ,"SUM", oBreak2,,,,.T.,.F.)
...
//³ Filtra dados    ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Processa( {|| AbrirQuery() }, "Aguarde...", "Filtrando registros...",.F.)
...
For nElem := 1 To Len(aDados)
	...
	cCliente := Substr(aDados[nElem][7],1,6) + "/" + Substr(aDados[nElem][7],7,2)
...
Aadd( aHelp, "Data de Referência: O usuário deve " )
Aadd( aHelp, "informar a data de referência e o " )
Aadd( aHelp, "sistema deve exibir o relatório " )
Aadd( aHelp, "através da data de referência " )
...
aHelp	:= {}
Aadd( aHelp, "escolher faixa de Clientes Ex: XXXX -YYYY ou" )
Aadd( aHelp, "escolher Clientes específicos como por Ex:" )
Aadd( aHelp, " Ex: XXX;YYY;AAA" )
PutSx1(cPerg,"04","Cliente      ?","","","mv_ch4","C",99,00,0,"G","","SA1","","","mv_par04","","","","","", "","","","","","","","","","","",aHelp,aHelp,aHelp)

aHelp	:= {}
Aadd( aHelp, "escolher faixa de gerentes Ex: XXXX - YYYY ou" )
Aadd( aHelp, "escolher gerentes específicos como por Ex:" )
Aadd( aHelp, " Ex: XXX;YYY;AAA" )
PutSx1(cPerg,"05","Gerente      ?","","","mv_ch5","C",99,00,0,"G","","SA3","","","mv_par05","","","","","", "","","","","","","","","","","",aHelp,aHelp,aHelp)

aHelp	:= {}
Aadd( aHelp, "escolher faixa de Supervisor Ex: XXXX - YYYY ou" )
Aadd( aHelp, "escolher Supervisor específicos como por" )
Aadd( aHelp, " Ex: XXX;YYY;AAA" )
PutSx1(cPerg,"06","Supervisor   ?","","","mv_ch6","C",99,00,0,"G","","SA3","","","mv_par06","","","","","", "","","","","","","","","","","",aHelp,aHelp,aHelp)

aHelp	:= {}
Aadd( aHelp, "escolher faixa de Vendedor Ex: XXXX - YYYY ou" )
Aadd( aHelp, "escolher Vendedor específicos como por Ex:" )
Aadd( aHelp, " Ex: XXX;YYY;AAA" )
PutSx1(cPerg,"07","Vendedor      ?","","","mv_ch7","C",99,00,0,"G","","SA3","","","mv_par07","","","","","", "","","","","","","","","","","",aHelp,aHelp,aHelp)
...
For nEmp := 1 To Len(aEmpresa)  
	...
	cQuery += "                F2_CLIENTE+F2_LOJA	                   AS CLIENTE, "
	...
	//cQuery += "                SF2.F2_EMISSAO BETWEEN '20111201' AND '20111231' "	
	cQuery += "                AND D2_TIPO NOT IN ('B','D','I') "
...
aTam:=TamSX3("A1_COD")
AADD(aCampos,{ "CLIENTE"   ,"C",09,aTam[2] } )
...
For nEmp := 1 To Len(aEmpresa)  
	...
	IncProc("Filtrando devoluções, aguarde..." )
	
	cQuery += " 		SELECT Substring(D1_DTDIGIT, 1, 6)         AS PERIOD, "
	cQuery += "                D1_FORNECE+D1_LOJA  		           AS FORNECE, "
	...
	If !Empty(MV_PAR04)
		If "-" $ Alltrim(MV_PAR04)
			cQuery += " 		   AND D1_FORNECE BETWEEN '"+Substr(MV_PAR04,1,6)+"' AND '"+Substr(MV_PAR04,8,6)+"' "
		Else
			cQuery += " 		   AND D1_FORNECE IN " + FormatIn(Alltrim(MV_PAR04),";")
		EndIf
	EndIf
...
While TRB->(!Eof())     
	...
	//³Se for administrador tudo					   ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	//If __cUserId $ cUserFil   //UPPER(cUserName) $ UPPER(cUserFil)
...
//If TFIL->(!Eof())
	...
	While TFIL->(!Eof())
		...
		If !Empty(MV_PAR06) //Filtro por supervidor
			If "-" $ Alltrim(MV_PAR06)
				If !(TFIL->A3_SUPER >= Substr(MV_PAR06,1,6) .And.  TFIL->A3_SUPER <= Substr(MV_PAR06,8,6))
					TFIL->(dbSkip())
					Loop
				EndIf
			Else
				If !Alltrim(TFIL->A3_SUPER) $ Alltrim(MV_PAR06)
					TFIL->(dbSkip())
					Loop
				EndIf
